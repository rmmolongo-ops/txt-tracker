    ...
    <div style={{ background: C.bg, minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <div style={{ background: C.card, borderRadius: 20, padding: 28, width: '100%', maxWidth: 380, border: '1px solid ' + C.border }}>
        ...
        <button onClick={handleAuth} disabled={loading || !email || !password}
          style={{ width: '100%', padding: 14, borderRadius: 12, border: 'none', cursor: loading ? 'not-allowed' : 'pointer', fontWeight: 800, fontSize: 16, background: (!email || !password) ? C.surface : 'linear-gradient(135deg, #3b82f6, #1d4ed8)', color: (!email || !password) ? C.muted : '#fff', transition: 'all 0.2s' }}>
